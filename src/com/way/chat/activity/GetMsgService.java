...
public class GetMsgService extends Service {
	...
	private static boolean isStart = false;// 是否与服务器连接上
	...
	public class FetchClient extends Thread{
		...
		public FetchClient(Client client,Handler mhandler){
			this.client = client;
...
//		isStart = client.start();
		fetchHandler();
		Thread t = new FetchClient( client, clientHandler);
		t.start();
		try {
			t.join();
